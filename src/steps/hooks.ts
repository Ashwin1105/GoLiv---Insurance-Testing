...
Before(async function () { browser = await chromium.launch({ headless: ({}).HEADLESS !== 'false', args: ['--no-sandbox', '--disable-dev-shm-usage'] }); context = await browser.newContext({ viewport: { width: 1366, height: 768 } }); const page = await context.newPage(); this.page = page; pf.page = page; await page.goto('http://host.docker.internal:5176'); await page.waitForLoadState('networkidle'); });
